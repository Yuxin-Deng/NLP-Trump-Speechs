...
#install packages you need to finish this homework. 
#packages needed: "tm", "SnowballCC", "RColorBrewer", "ggplot2", "wordcloud",
...
cname <- file.path("/Users/dengyuxin/Downloads", "texts")   
cname   
dir(cname)
...
head(table(freq), 20) #The resulting output is two rows of numbers. The top number is the frequency with which words appear and the bottom number reflects how many words appear that frequently.
#Here, considering only the 20 lowest word frequencies, we can see that 1602 terms appear only once. There are also a lot of others that appear very infrequently.
...
p <- ggplot(subset(wf, freq>50), 
            ...
            labs(title="Histogram of Word Frequency (>=50)", x="Words", y="Frequency")+
...
library(tm)
findAssocs(dtm, c("people","country","america","american","right","immigration","law","jobs"), corlimit=0.9) # specifying a correlation limit of 0.85. Feel free to change the words you want to check and correlation limit

#####Create word clouds######
# First load the package that makes word clouds in R.    
library(wordcloud)  
dtms <- removeSparseTerms(dtm, 0.2) # Prepare the data (max 20% empty space)   
freq <- colSums(as.matrix(dtm)) # Find word frequencies   
dark2 <- brewer.pal(6, "Dark2")   
wordcloud(names(freq), freq, max.words=150, rot.per=0.15,colors=dark2,random.order=FALSE)    



#####Hierarchal Clustering#####   
dtms <- removeSparseTerms(dtm, 0.2) # This makes a matrix that is only 20% empty space.
library(cluster)   
...
library(fpc)   
library(cluster)  
dtms <- removeSparseTerms(dtm, 0.2) # Prepare the data (max 15% empty space)   
